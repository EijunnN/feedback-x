...
export function ProjectSettingsForm({
  ...
  const [widgetColor, setWidgetColor] = useState(project.widgetColor || "#ea580c");
  const [widgetPosition, setWidgetPosition] = useState(project.widgetPosition || "bottom-right");
  const [widgetText, setWidgetText] = useState(project.widgetText || "Feedback");
  ...
  return (
    <form action={updateAction} className="grid xl:grid-cols-[1fr_400px] gap-8">
      <input type="hidden" name="id" value={project.id} />
      
      {/* Hidden inputs for non-native components */}
      <input type="hidden" name="widgetPosition" value={widgetPosition} />
      <input type="hidden" name="enableBugs" value={String(enableBugs)} />
      <input type="hidden" name="enableIdeas" value={String(enableIdeas)} />
      <input type="hidden" name="enableOther" value={String(enableOther)} />
      <input type="hidden" name="showBranding" value={String(showBranding)} />
      ...
      <div className="space-y-8 w-full">
          ...
          <section className="border border-zinc-800 rounded-sm bg-zinc-950/50 p-8">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <label htmlFor="domain" className="text-[10px] font-mono uppercase text-zinc-500 tracking-wider">
                  Target Domain
                </label>
                <Input
                  id="domain"
          ...
          <section className="border border-zinc-800 rounded-sm bg-zinc-950/50 p-8">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {[
                  { id: 'enableBugs', label: 'Bug Reports', state: enableBugs, set: setEnableBugs },
                  { id: 'enableIdeas', label: 'Feature Ideas', state: enableIdeas, set: setEnableIdeas },
                  { id: 'enableOther', label: 'General Input', state: enableOther, set: setEnableOther },
              ].map(item => (
                <div 
                  key={item.id} 
                  className="flex items-center gap-3 p-4 border border-zinc-800 rounded-sm bg-black hover:border-zinc-700 transition-colors"
                >
                    <Checkbox 
                      id={item.id}
                      checked={item.state}
                    ...
                    />
                    <label htmlFor={item.id} className="text-xs text-zinc-300 font-mono cursor-pointer uppercase tracking-wide">
                      {item.label}
                    </label>
                </div>
              ))}
          ...
          <section className="border border-zinc-800 rounded-sm bg-zinc-950/50 p-8">
            ...
            {planDetails.allowCustomization ? (
              <div className="grid md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label htmlFor="widgetColor" className="text-[10px] font-mono uppercase text-zinc-500 tracking-wider">
                    Accent Hex
                  </label>
                  <div className="flex gap-2">
                    <div className="relative">
                      ...
                      />
                      <div className="w-10 h-10 border border-zinc-800 rounded-sm" style={{ backgroundColor: widgetColor }} />
                    </div>
                    <Input
                      type="text"
                      value={widgetColor}
                ...
                <div className="space-y-2">
                  <label htmlFor="widgetText" className="text-[10px] font-mono uppercase text-zinc-500 tracking-wider">
                    Trigger Label
                  </label>
                  <Input
                    id="widgetText"
                ...
                <div className="space-y-2 flex items-end pb-2">
                    <div className="flex items-center gap-3">
                        <Checkbox
                          id="showBranding"
                          checked={showBranding}
                          onCheckedChange={(c) => setShowBranding(c as boolean)}
                          disabled={!planDetails.allowRemoveBranding}
                        />
                        <div>
                          <label htmlFor="showBranding" className="text-xs text-zinc-300 font-mono uppercase tracking-wide block">
                            Vendor Signature
                          </label>
                          {!planDetails.allowRemoveBranding && (
                             <span className="text-[10px] text-zinc-600 font-mono">Requires MAX plan</span>
                          )}
                        </div>
                    </div>
                </div>
              ...
              <div className="rounded-sm border border-dashed border-zinc-800 bg-zinc-900/10 p-8 text-center">
                ...
                <Button asChild size="sm" className="bg-white text-black hover:bg-zinc-200 font-mono text-xs uppercase rounded-sm font-bold">
      ...
      <div className="space-y-6">
        <section className="sticky top-6">
            <h2 className="text-sm font-bold text-white uppercase font-mono tracking-wider mb-4 pl-1">Live Preview</h2>
            <WidgetPreview
              color={planDetails.allowCustomization ? widgetColor : "#ea580c"}
              position={planDetails.allowCustomization ? widgetPosition : "bottom-right"}
              text={planDetails.allowCustomization ? widgetText : "Feedback"}
              showBranding={planDetails.allowRemoveBranding ? showBranding : true}
              enableBugs={enableBugs}
              enableIdeas={enableIdeas}
            ...
            <div className="mt-8">
                <Button type="submit" className="w-full bg-orange-600 hover:bg-orange-500 text-white font-mono text-xs uppercase font-bold h-12 rounded-sm shadow-[0_0_20px_rgba(234,88,12,0.15)] border border-orange-500">
